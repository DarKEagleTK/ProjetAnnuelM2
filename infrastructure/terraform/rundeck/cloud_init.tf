...
data "template_file" "user_data" {
  ...
  vars = {
    hostname = "serv-pa-rundeck-00${count.index}"
    fqdn = "serv-pa-rundeck-00${count.index}"
  }
...
resource "libvirt_cloudinit_disk" "commoninit" {
  name = "serv-pa-rundeck-00${count.index}-commoninit.iso"
  ...
  depends_on = [ libvirt_pool.pa-rundeck ]
}